#!/usr/bin/env bash
# Testa o parser Python com os casos .c e compara a saída com o resultado esperado.
# Uso: ./testar_parser_c.sh [diretorio_dos_testes] [script_do_parser]
# Exemplo: ./testar_parser_c.sh ./testes-parser-50 ./parser.py
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/parser-py-XXXXXX")"
...
for source in "${CASES[@]}"; do
    ...
    set +e; python3 "$PARSER" "$source" >"$out" 2>"$err"; status=$?; set -e
